...
/**
 * User joins an existing Space.
 * @param {string} joinCode - The unique code for the space.
 * @param {string} username - The desired username for the user in that space.
 * @returns {Promise<object>} Response data from the API.
 */
export const joinSpace = async (joinCode, username) => {
  try {
 ...
 * @param {string} adminPassword - The admin's chosen password.
 * @param {string} confirmPassword - Confirmation of the admin's password.
 * @returns {Promise<object>} Response data from the API.
 */
 ...
 * @param {string} joinCode - The unique code for the space.
 * @param {string} adminPassword - The admin's password for that space.
 * @returns {Promise<object>} Response data from the API.
 */
...
/**
 * User logs into an existing Space (for subsequent logins after joining).
 * @param {string} joinCode - The unique code for the space.
 * @param {string} username - The user's username in that space.
 * @returns {Promise<object>} Response data from the API.
 */
export const userLoginToSpace = async (joinCode, username) => {
  try {
...
/**
 * Admin manually sets a user's overall score.
 * The backend uses the admin's authenticated spaceId from their token.
 * @param {string|number} userIdToEdit The ID of the user whose score is being changed.
 * @param {number} newScore The new overall score.
 * @returns {Promise<object>}
 */
export const setOverallScoreByAdmin = async (userIdToEdit, newScore) => {
    try {
        // Backend route is PUT /api/admin/users/:userIdToEdit/score
        const response = await axiosInstance.put(`/admin/users/${userIdToEdit}/score`, { newScore: parseInt(newScore, 10) });
        return response.data; // Expected: { message: "User score updated..." }
    } catch (error) {
