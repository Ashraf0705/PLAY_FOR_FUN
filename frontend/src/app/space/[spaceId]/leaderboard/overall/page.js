...
export default function OverallLeaderboardPage() {
    ...
    if (isAuthLoading) { /* ... same loading as weekly ... */ }
    if (!isLoggedIn || !authUser || authUser.space_id?.toString() !== currentSpaceIdFromUrl) { /* ... same unauthorized as weekly ... */ }

    // More sophisticated rank calculation considering ties
    let lastScoreOverall = null;
    let currentRankOverall = 0;
    let playersAtCurrentRankOverall = 0;

    const processedLeaderboardOverall = leaderboard.map((player, index) => {
    ...
    return (
        <SpaceLayout>
            <div className="max-w-3xl mx-auto p-4 md:p-8"> {/* Increased overall padding */}
                ...
                {!isLoadingData && !error && processedLeaderboardOverall.length > 0 && (
                    <div className="overflow-x-auto"> {/* Added for responsiveness on small screens */}
                            ...
                            <tbody>
                                {processedLeaderboardOverall.map((player) => (
                                    <tr key={player.user_id} 
                                        className={player.user_id === authUser.user_id ? 'highlighted-row' : ''}>
